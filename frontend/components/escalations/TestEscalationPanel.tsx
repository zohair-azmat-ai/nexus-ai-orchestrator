...
import { createTestEscalation } from "@/lib/escalations";
...
export default function TestEscalationPanel() {
  ...
  const [result, setResult] = useState<{ ok: true; caseId: string } | { ok: false; error: string } | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const created = await createTestEscalation({ customer_ref: customerRef, message, severity, channel });
      setResult({ ok: true, caseId: created.case_id });
      setCustomerRef("");
      setMessage("");
      setSeverity("medium");
      setChannel("test");
    ...
    <section className="rounded-2xl border border-dashed border-white/10 bg-slate-950/60 p-5">
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="flex items-end">
          ...
          >
            {loading ? "Creating…" : "Create Test Escalation"}
          </button>
        </div>
