...
class JobManager:
    async def submit(
        ...
        job_row = await crud.create_job(db, job_type=job_type, payload=payload)
        event_logger.emit(EVENT_JOB_CREATED, job_id=job_row.id, job_type=job_type)
        logger.info("job.created", extra={"job_id": job_row.id, "job_type": job_type})

        if settings.jobs_inline_mode:
            await self._execute(db, job_row.id, job_type, payload)
        else:
            asyncio.create_task(
                self._run_background(job_row.id, job_type, payload)
            )
    ...
    async def _execute(
        ...
        event_logger.emit(EVENT_JOB_STARTED, job_id=job_id, job_type=job_type)
        ...
        try:
            ...
            event_logger.emit(EVENT_JOB_COMPLETED, job_id=job_id, job_type=job_type)
            ...
            event_logger.emit(EVENT_JOB_FAILED, job_id=job_id, job_type=job_type, error=error_msg)
    ...
    async def _run_background(
        ...
        async with session_factory() as db:
            try:
                await self._execute(db, job_id, job_type, payload)
                await db.commit()
            except Exception as exc:
                await db.rollback()
                logger.error(
